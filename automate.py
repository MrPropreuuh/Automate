...
def verifier_disponibilite_kit_et_maj_comptes(donnees):
    kit_disponible = False
    ...
    # Si le kit est disponible, réinitialiser le champ `today` pour tous les comptes
    if kit_disponible:
        for compte in donnees["comptes"]:
            compte["today"] = "false"
        # N'oubliez pas de sauvegarder les changements après la mise à jour
        sauvegarder_donnees(donnees)

    return kit_disponible
...
def main():
    ...
    if verifier_disponibilite_kit_et_maj_comptes(donnees):
        ...
        print("Arrêt du script car le kit n'est pas encore disponible.")
